...
def scan(folder: Path) -> None:
    for item in folder.iterdir():
        # Якщо це папка то додаємо її  до списпку фолдерс і переходимо до наступного елемента папки
        if item.is_dir():
            # перевіряємо щоб папка не була тією в які ми складаємо вже файли
            if item.name not in ('archives', 'video', 'audio', 'documents', 'images', 'MY_OTHER'):
                FOLDERS.append(item)
                # скануємо вкладену папку 
                scan(item) # рекурсія
            continue # переходимо до настпуного елементу в сканованій папці
...
if __name__=="__main__":
    ...
    print(f'Types of files in folder:{EXTENSTION}')
